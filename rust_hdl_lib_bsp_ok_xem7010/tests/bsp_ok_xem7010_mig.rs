...
use rust_hdl_lib_bsp_ok_xem7010::xem7010::mcb_if::MCBInterface4GDDR3;
use rust_hdl_lib_bsp_ok_xem7010::xem7010::mig7::MemoryInterfaceGenerator7Series;
    ...
    fn update(&mut self) {
        ...
        match self.state.q.val() {
            MIGState::Idle => {
                ...
                if self.start_cmd.trigger.val().get_bit(1) {
            ...
            MIGState::WaitAck => {
                self.mig.command.next = 0.into(); // This is a write command
            ...
            MIGState::WaitRead => {
                ...
                if self.mig.read_data_valid.val() {
                    self.state.d.next = MIGState::Idle;
                    self.cmd_done.trigger.next = 2.into();
                }
            }
            _ => {
                self.state.d.next = MIGState::Idle;
