...
fn test_rising_edge_detector_works() {
    ...
    sim.add_clock(5, |x : &mut EdgeDetector| x.clock.next = !x.clock.val());
    ...
    sim.run_traced(uut, 1000, std::fs::File::create("edge_det2.vcd").unwrap()).unwrap();
}
...
fn test_falling_edge_detector_works() {
    ...
    sim.run_traced(uut, 1000, std::fs::File::create("edge_det.vcd").unwrap()).unwrap();
}