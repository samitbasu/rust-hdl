...
use crate::capacitors::{map_three_digit_cap_to_uf, CapacitorTolerance, map_pf_to_label, CapacitorKind};
...
fn match_hxd_esr(voltage: f64, cap_pf: f64, size_code: &str) -> i32 {
    ...
    match (voltage, cap, size_code) {
        ...
        (63,100.,"JA0") => 30,
        (80,56.,"JA0") => 33,
        _ => unimplemented!("Unrecognized voltage, cap size ({} {} {}) triplet", voltage, cap, size_code),
    }
}
...
pub fn make_nippon_hxd_capacitor(part_number: &str) -> Capacitor {
    ...
    let description = format!("United Chemi-Con HXD Seris Alum Poly SMD {} {}",
        size, label);
    Capacitor {
        details: PartDetails {
            label,
            manufacturer: Manufacturer {name: "United Chemi-Con".to_string(),
