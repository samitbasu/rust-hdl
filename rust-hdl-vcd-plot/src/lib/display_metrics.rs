...
use crate::{TimedValue, TimeView, VCDStyle};
use crate::renderable::Renderable;
use crate::utils::{line, rect};
...
impl DisplayMetrics {
    ...
    pub(crate) fn timescale(&self, mut document: SVG) -> SVG {
        ...
        for major in first_major_tick..=last_major_tick {
            ...
            if let Some(label) = self.timescale_major_label(major, &crate::time_label(delt * major)) {
    ...
    pub(crate) fn vector_signal_plot<T: PartialEq + Clone + Renderable>(
        ...
        let values = crate::changes(values);
