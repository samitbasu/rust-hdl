...
use crate::xem6010::mig::MIGInstruction;
...
impl Logic for DDRFIFO {
    ...
    fn update(&mut self) {
        ...
        self.mig.p0_cmd.cmd.next.burst_len = 31.into(); // Always work with 32 word packets
        ...
        match self.state.q.val() {
            ...
            DDRFIFOState::ReadComplete => {
                self.read_address.d.next = self.read_address.q.val() + 128;
                self.state.d.next = DDRFIFOState::Busy;
            }
            DDRFIFOState::Write => {
            ...
            DDRFIFOState::UpdateWriteAddress => {
                self.write_address.d.next = self.write_address.q.val() + 128;
                self.state.d.next = DDRFIFOState::Busy;
            }
            DDRFIFOState::Busy => {
        ...
        self.status.next = bit_cast::<8, 1>(self.mig.p0_wr.error.val().into())
            | (bit_cast::<8, 1>(self.mig.p0_wr.underrun.val().into()) << 1)
            | (bit_cast::<8, 1>(self.mig.p0_cmd.full.val().into()) << 2)
            | (bit_cast::<8, 1>(self.mig.p0_rd.error.val().into()) << 3)
            | (bit_cast::<8, 1>(self.mig.p0_rd.overflow.val().into()) << 4)
            | (bit_cast::<8, 1>(self.have_data.val().into()) << 5)
            | (bit_cast::<8, 1>(true.into()) << 7);
    }
}
