...
impl Logic for MemoryInterfaceGenerator {
    fn update(&mut self) {
        ...
        if self.timer.q.val().any() {
            self.timer.d.next = self.timer.q.val() - 1;
        } else {
        ...
        match self.state.q.val() {
            State::Init => {
                ...
                self.timer.d.next = 100.into();
            }
            State::Calibrating => {
                if self.timer.q.val() == 0 {
                    ...
                    if self.cmd.val().byte_address.get_bits::<2>(0).any() {
                        ...
                        match self.cmd.val().instruction {
                            MIGInstruction::Write | MIGInstruction::WritePrecharge => {
                                ...
                                self.timer.d.next = bit_cast::<16, 6>(self.cmd.val().burst_len) + 1;
                                self.address.d.next =
                                    bit_cast::<32, 30>(self.cmd.val().byte_address) >> 2;
