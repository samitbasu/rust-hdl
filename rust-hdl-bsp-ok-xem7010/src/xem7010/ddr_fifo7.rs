...
impl<const N: usize> Logic for DDR7FIFO<N> {
    ...
    fn update(&mut self) {
        ...
        match self.state.q.val() {
            ...
            DDR7FIFOState::Write => {
                self.mig.command.next = 0.into(); // Write command
                self.mig.enable.next = true;
                self.mig.address.next = bit_cast::<29, 28>(self.write_address.q.val());
                if self.mig.ready.val() {
                    self.write_address.d.next = self.write_address.q.val() + 8; // This is the number of 16 bit words
                    self.state.d.next = DDR7FIFOState::Idle;
                }
            }
            DDR7FIFOState::Read => {
                ...
                if self.mig.read_data_valid.val() {
                    self.read_address.d.next = self.read_address.q.val() + 8;
                    self.state.d.next = DDR7FIFOState::Idle;
                }
            }
            _ => {
                self.state.d.next = DDR7FIFOState::Booting;
            }
        }
        self.status.next = bit_cast::<8, 1>(self.front_porch.empty.val().into())
            | (bit_cast::<8, 1>(self.front_porch.full.val().into()) << 1)
            | (bit_cast::<8, 1>(self.back_porch.empty.val().into()) << 2)
            | (bit_cast::<8, 1>(self.back_porch.full.val().into()) << 3)
            | (bit_cast::<8, 1>(self.is_empty.val().into()) << 4)
            | (bit_cast::<8, 1>((self.state.q.val() == DDR7FIFOState::Idle).into()) << 5)
            | (bit_cast::<8, 1>(self.mig.ready.val().into()) << 6);
    }
}
