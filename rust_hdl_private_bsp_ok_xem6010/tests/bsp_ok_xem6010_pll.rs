...
use rust_hdl_private_bsp_ok_xem6010::xem6010::pll::{PLLFreqSynthesis, Spartan6PLLSettings};
...
impl Logic for OpalKellyPLLTest {
    ...
    fn update(&mut self) {
        ...
        self.led.next = bit_cast::<8, 1>(self.slow_pulser.pulse.val().into())
            | (bit_cast::<8, 1>(self.fast_pulser.pulse.val().into()) << 5)
            | (bit_cast::<8, 1>(self.med_pulser.pulse.val().into()) << 7);
    }
}
