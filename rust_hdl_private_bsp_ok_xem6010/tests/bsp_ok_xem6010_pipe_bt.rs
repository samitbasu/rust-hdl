...
use rust_hdl_private_bsp_ok_xem6010::xem6010::pins::{xem_6010_base_clock, xem_6010_leds};
use rust_hdl_private_bsp_ok_xem6010::xem6010::XEM6010;
use rust_hdl_private_ok_core::core::prelude::*;
use rust_hdl_private_ok_core::frontpanel::{make_u16_buffer, OkError};
...
impl Logic for OpalKellyBTPipeOutTest {
    ...
    fn update(&mut self) {
        ...
        self.led.next = !(bit_cast::<8, 1>(self.fifo_out.empty.val().into())
            | (bit_cast::<8, 1>(self.fifo_out.full.val().into()) << 1)
            | (bit_cast::<8, 1>(self.fifo_out.almost_empty.val().into()) << 2)
            | (bit_cast::<8, 1>(self.fifo_out.almost_full.val().into()) << 3)
            | (bit_cast::<8, 1>(self.fifo_out.overflow.val().into()) << 4)
            | (bit_cast::<8, 1>(self.fifo_out.underflow.val().into()) << 5));
    }
}
...
fn test_opalkelly_xem_6010_btpipe_runtime() -> Result<(), OkError> {
    let hnd = ok_test_prelude(
        target_path!("xem_6010/btpipe/top.bit"),
        env!("XEM6010_SERIAL"),
    )?;
