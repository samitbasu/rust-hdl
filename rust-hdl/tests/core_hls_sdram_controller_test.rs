...
fn test_sdram_controller_boots() {
    ...
    sim.add_testbench(move |mut sim: Sim<HostSDRAMControllerTest>| {
        ...
        hls_host_write!(sim, bidi_clock, x, pc_to_host, 0, [0x1234, 0x5678, 0xDEAD, 0xBEEF]);
        // Write the address
        ...
        hls_host_write!(sim, bidi_clock, x, pc_to_host, 0, [0xCAFE, 0xBABE, 0xEFFE, 0x5EA1]);
    ...
    sim.run_to_file(Box::new(uut), 10_000_000,
                    &vcd_path!("hls_sdram_controller.vcd")).unwrap();
}