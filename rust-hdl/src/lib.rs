...
//! Matches are used for state machines and implementing ROMs.  Here is an example of a Match
//! for a state machine:
//! ```rust
//! # use rust_hdl::core::prelude::*;
//! # use rust_hdl::widgets::prelude::*;
...
//!                      self.state.d.next = State::Running;
//!                   }
//!            _ => self.state.d.next = State::Idle
//!        }
//!        if self.stop.val() {
...
//!   #[hdl_gen]
//!   fn update(&mut self) {
//!        self.output.next = D::default(); // All synthesizable types must have a default...
//!        for i in 0..N {
//!           if self.select.val().index() == i {
//!              self.out.next = self.input_lines[i].val();
//!           }
//!        }
//!    }
//! }
...
//!                 }
//!             }
//!             // This is a peculiar requirement for HDL statemachines.  Even if the
//!             // enum is exhaustively matched, you need to include a default here.
//!             _ => {
